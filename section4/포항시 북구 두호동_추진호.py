...
for first_item in soup.find_all("data"):          #@ data 전부 찾아라. -> 그래서 변수 location에 담아줘
    ...
    # complete='포항시 북구 두호동의 3월'+day_item+'일'+time_item+'시 기온은'+temp_item+'도 입니다.'
    # complete='포항시 북구 두호동의 3월{}일 {}시 기온은 {}도 입니다.'.format(str(int(day_item)+1),time_item,temp_item)
    # f.write('포항시 북구 두호동의 3월{}일 {}시 기온은 {}도 입니다.'.format(str(int(day_item)+1),time_item,temp_item))
    # if not (complete in info):
    #     info[complete]=[]
    # for a in complete:
    #     info[complete].append(a)
